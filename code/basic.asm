...
BLKDAT  = $0400                         ;Buffer for sector transfer
;	* = $0600
;256 bytes of RAM for SYSTEM USE (added by MW)
BLKENDL = $0600                         ;Buffer ends here (byte)
SYSTICK = $0601						    ;Systick timer (word)
RTCTICK = $0603						    ;RTC tick timer/uptime (word)
...
LAB_16F4
	...
	!8	$2C			; makes next line BIT LAB_0EA2
...
LAB_DEC
	...
	!8	$2C			; BIT abs to skip the LDA below
...
LAB_18E0
	LDA	#$20			; load " "
	!8	$2C			; change next line to BIT LAB_3FA9
...
LAB_1E85
	...
	!8	$2C			; makes next bit BIT LAB_08A2
...
CFCHERR
	...
	BEQ	CFNERR
    JSR	LAB_CRLF		;print CR/LF
	LDA	#<LAB_CFMSG1	;point to memory size message (low addr)
	LDY	#>LAB_CFMSG1	;point to memory size message (high addr)
	JSR	LAB_18C3		;print null terminated string from memory
	SED				    ;need decimal mode for nibble convert
	LDA	CFREG1		    ;get error code
	JSR	LAB_A2HX        ;convert A to ASCII hex byte and output
	CLD				    ;back to binary    
CFNERR
    RTS    
...
KBDENDEVS:	
	...
	STA KBD_CMD
	JSR KBDWAITINBUF			 ;Send actual configuration byte
	LDA #$68					 ;Interrupts enabled, system flag reset, first port clock enabled
	STA KBD_DATA				 ;second port clock enabled, first port translation disabled    
KBD_OK:	
	;LDA #$00                    ;Zero KBDDATA
	;STA KBDDATA
...
KBDRCV:
    LDA KBD_STATUS             ;Load status register of keyboard controller
    AND #$01                   ;Check if there is any data to read
    BNE KBDRCV_LD              ;If there is, read actual data
    RTS
KBDRCV_LD:
	PHP
    TYA
...
PG2_TABS
	!8	$00			; ctrl-c flag		-	$00 = enabled
	!8	$00			; ctrl-c byte		-	GET needs this
	!8	$00			; ctrl-c byte timeout	-	GET needs this
	!16	CTRLC			; ctrl c check vector
;	!16	xxxx			; non halting key input	-	monitor to set this
;	!16	xxxx			; output vector		-	monitor to set this
;	!16	xxxx			; load vector		-	monitor to set this
;	!16	xxxx			; save vector		-	monitor to set this
PG2_TABE
...
StrTab
	!8	$4C			; JMP opcode
	!16 LAB_COLD		; initial warm start vector (cold start)

	!8	$00			; these bytes are not used by BASIC
	!16	$0000			; 
	!16	$0000			; 
	!16	$0000			; 

	!8	$4C			; JMP opcode
	!16	LAB_FCER		; initial user function vector ("Function call" error)
	!8	$00			; default NULL count
	!8	$00			; clear terminal position
	!8	$00			; default terminal width byte
	!8	$F2			; default limit for TAB = 14
	!16	Ram_base		; start of user RAM
EndTab
...
LAB_25AD
	!8	$80,$35,$04,$F3	; 0.70711	1/root 2
LAB_25B1
	!8	$81,$35,$04,$F3	; 1.41421	root 2
LAB_25B5
	!8	$80,$80,$00,$00	; -0.5
LAB_25B9
	!8	$80,$31,$72,$18	; 0.69315	LOG(2)

					; numeric PRINT constants
LAB_2947
	!8	$91,$43,$4F,$F8	; 99999.9375 (max value with at least one decimal)
LAB_294B
	!8	$94,$74,$23,$F7	; 999999.4375 (max value before scientific notation)
LAB_294F
	!8	$94,$74,$24,$00	; 1000000

					; EXP(n) constants and series
LAB_2AFA
	!8	$81,$38,$AA,$3B	; 1.4427	(1/LOG base 2 e)
LAB_2AFE
	!8	$06			; counter
	!8	$74,$63,$90,$8C	; 2.17023e-4
	!8	$77,$23,$0C,$AB	; 0.00124
	!8	$7A,$1E,$94,$00	; 0.00968
	!8	$7C,$63,$42,$80	; 0.05548
	!8	$7E,$75,$FE,$D0	; 0.24023
	!8	$80,$31,$72,$15	; 0.69315
	!8	$81,$00,$00,$00	; 1.00000

;##	!8	$07			; counter
;##	!8	$74,$94,$2E,$40	; -1/7! (-1/5040)
;##	!8	$77,$2E,$4F,$70	;  1/6! ( 1/720)
;##	!8	$7A,$88,$02,$6E	; -1/5! (-1/120)
;##	!8	$7C,$2A,$A0,$E6	;  1/4! ( 1/24)
;##	!8	$7E,$AA,$AA,$50	; -1/3! (-1/6)
;##	!8	$7F,$7F,$FF,$FF	;  1/2! ( 1/2)
;##	!8	$81,$80,$00,$00	; -1/1! (-1/1)
;##	!8	$81,$00,$00,$00	;  1/0! ( 1/1)

					; trigonometric constants and series
LAB_2C78
	!8	$81,$49,$0F,$DB	; 1.570796371 (pi/2) as floating #
LAB_2C84
	!8	$04			; counter
	!8	$86,$1E,$D7,$FB	; 39.7109
;##	!8	$86,$1E,$D7,$BA	; 39.7109
	!8	$87,$99,$26,$65	;-76.575
;##	!8	$87,$99,$26,$64	;-76.575
	!8	$87,$23,$34,$58	; 81.6022
	!8	$86,$A5,$5D,$E1	;-41.3417
;##	!8	$86,$A5,$5D,$E0	;-41.3417
LAB_2C7C
	!8	$83,$49,$0F,$DB	; 6.28319 (2*pi) as floating #
;##	!8	$83,$49,$0F,$DA	; 6.28319 (2*pi) as floating #

LAB_2CC9
	!8	$08			; counter
	!8	$78,$3A,$C5,$37	; 0.00285
	!8	$7B,$83,$A2,$5C	;-0.0160686
	!8	$7C,$2E,$DD,$4D	; 0.0426915
	!8	$7D,$99,$B0,$1E	;-0.0750429
	!8	$7D,$59,$ED,$24	; 0.106409
	!8	$7E,$91,$72,$00	;-0.142036
	!8	$7E,$4C,$B9,$73	; 0.199926
	!8	$7F,$AA,$AA,$53	;-0.333331
...
LAB_259C
	!8	$81,$00,$00,$00	; 1.000000, used for INC
LAB_2AFD
	!8	$81,$80,$00,$00	; -1.00000, used for DEC. must be on the same page as +1.00

					; misc constants
LAB_1DF7
	!8	$90			;-32768 (uses first three bytes from 0.5)
LAB_2A96
	!8	$80,$00,$00,$00	; 0.5
LAB_2C80
	!8	$7F,$00,$00,$00	; 0.25
LAB_26B5
	!8	$84,$20,$00,$00	; 10.0000 divide by 10 constant
...
LAB_CTBL
	...
	!16	LAB_NEXT-1		; NEXT
	!16	LAB_DATA-1		; DATA
	!16	LAB_INPUT-1		; INPUT
	!16	LAB_DIM-1		; DIM
	!16	LAB_READ-1		; READ
	!16	LAB_LET-1		; LET
	!16	LAB_DEC-1		; DEC			new command
	!16	LAB_GOTO-1		; GOTO
	!16	LAB_RUN-1		; RUN
	!16	LAB_IF-1		; IF
	!16	LAB_RESTORE-1	; RESTORE		modified command
	!16	LAB_GOSUB-1		; GOSUB
	!16	LAB_RETIRQ-1	; RETIRQ		new command
	!16	LAB_RETNMI-1	; RETNMI		new command
	!16	LAB_RETURN-1	; RETURN
	!16	LAB_REM-1		; REM
	!16	LAB_STOP-1		; STOP
	!16	LAB_ON-1		; ON			modified command
	!16	LAB_NULL-1		; NULL		modified command
	!16	LAB_INC-1		; INC			new command
	!16	LAB_WAIT-1		; WAIT
	!16	V_LOAD-1		; LOAD
	!16	V_SAVE-1		; SAVE
	!16	LAB_DEF-1		; DEF
	!16	LAB_POKE-1		; POKE
	!16	LAB_DOKE-1		; DOKE		new command
	!16	LAB_CALL-1		; CALL		new command
	!16	LAB_DO-1		; DO			new command
	!16	LAB_LOOP-1		; LOOP		new command
	!16	LAB_PRINT-1		; PRINT
	!16	LAB_CONT-1		; CONT
	!16	LAB_LIST-1		; LIST
	!16	LAB_CLEAR-1		; CLEAR
	!16	LAB_NEW-1		; NEW
	!16	LAB_WDTH-1		; WIDTH		new command
	!16	LAB_GET-1		; GET			new command
	!16	LAB_SWAP-1		; SWAP		new command
	!16	LAB_BITSET-1	; BITSET		new command
	!16	LAB_BITCLR-1	; BITCLR		new command
	!16	LAB_IRQ-1		; IRQ			new command
	!16	LAB_NMI-1		; NMI			new command

; function pre process routine table

LAB_FTPL
LAB_FTPM	= LAB_FTPL+$01
	!16	LAB_PPFN-1		; SGN(n)	process numeric expression in ()
	!16	LAB_PPFN-1		; INT(n)		"
	!16	LAB_PPFN-1		; ABS(n)		"
	!16	LAB_EVEZ-1		; USR(x)	process any expression
	!16	LAB_1BF7-1		; FRE(x)		"
	!16	LAB_1BF7-1		; POS(x)		"
	!16	LAB_PPFN-1		; SQR(n)	process numeric expression in ()
	!16	LAB_PPFN-1		; RND(n)		"
	!16	LAB_PPFN-1		; LOG(n)		"
	!16	LAB_PPFN-1		; EXP(n)		"
	!16	LAB_PPFN-1		; COS(n)		"
	...
	!16	LAB_PPFN-1		; TAN(n)		"
	!16	LAB_PPFN-1		; ATN(n)		"
	!16	LAB_PPFN-1		; PEEK(n)		"
	!16	LAB_PPFN-1		; DEEK(n)		"
	!16	$0000			; SADD()	none
	!16	LAB_PPFS-1		; LEN($)	process string expression in ()
	!16	LAB_PPFN-1		; STR$(n)	process numeric expression in ()
	!16	LAB_PPFS-1		; VAL($)	process string expression in ()
	!16	LAB_PPFS-1		; ASC($)		"
	!16	LAB_PPFS-1		; UCASE$($)		"
	!16	LAB_PPFS-1		; LCASE$($)		"
	!16	LAB_PPFN-1		; CHR$(n)	process numeric expression in ()
	!16	LAB_BHSS-1		; HEX$(n)		"
	!16	LAB_BHSS-1		; BIN$(n)		"
	!16	$0000			; BITTST()	none
	!16	LAB_MMPP-1		; MAX()	process numeric expression
	!16	LAB_MMPP-1		; MIN()		"
	!16	LAB_PPBI-1		; PI		advance pointer
	!16	LAB_PPBI-1		; TWOPI		"
	!16	$0000			; VARPTR()	none
	!16	LAB_LRMS-1		; LEFT$()	process string expression
	!16	LAB_LRMS-1		; RIGHT$()		"
	!16	LAB_LRMS-1		; MID$()		"
...
LAB_FTBM	= LAB_FTBL+$01
	...
	!16	LAB_POS-1		; POS()
	!16	LAB_SQR-1		; SQR()
	!16	LAB_RND-1		; RND()		modified function
	!16	LAB_LOG-1		; LOG()
	!16	LAB_EXP-1		; EXP()
	!16	LAB_COS-1		; COS()
	!16	LAB_SIN-1		; SIN()
	...
	!16	LAB_ATN-1		; ATN()
	!16	LAB_PEEK-1		; PEEK()
	!16	LAB_DEEK-1		; DEEK()		new function
	!16	LAB_SADD-1		; SADD()		new function
	!16	LAB_LENS-1		; LEN()
	!16	LAB_STRS-1		; STR$()
	!16	LAB_VAL-1		; VAL()
	!16	LAB_ASC-1		; ASC()
	!16	LAB_UCASE-1		; UCASE$()		new function
	!16	LAB_LCASE-1		; LCASE$()		new function
	!16	LAB_CHRS-1		; CHR$()
	!16	LAB_HEXS-1		; HEX$()		new function
	!16	LAB_BINS-1		; BIN$()		new function
	!16	LAB_BTST-1		; BITTST()		new function
	!16	LAB_MAX-1		; MAX()		new function
	!16	LAB_MIN-1		; MIN()		new function
	!16	LAB_PI-1		; PI			new function
	!16	LAB_TWOPI-1		; TWOPI		new function
	!16	LAB_VARPTR-1	; VARPTR()		new function
	!16	LAB_LEFT-1		; LEFT$()
	!16	LAB_RIGHT-1		; RIGHT$()
	!16	LAB_MIDS-1		; MID$()
...
TAB_1STC
	...
	!8	"W"
	!8	"^"
	!8	$00			; table terminator

; pointers to keyword tables

TAB_CHRT
	!16	TAB_STAR		; table for "*"
	!16	TAB_PLUS		; table for "+"
	!16	TAB_MNUS		; table for "-"
	!16	TAB_SLAS		; table for "/"
	!16	TAB_LESS		; table for "<"
	!16	TAB_EQUL		; table for "="
	!16	TAB_MORE		; table for ">"
	!16	TAB_QEST		; table for "?"
	!16	TAB_ASCA		; table for "A"
	!16	TAB_ASCB		; table for "B"
	!16	TAB_ASCC		; table for "C"
	!16	TAB_ASCD		; table for "D"
	!16	TAB_ASCE		; table for "E"
	!16	TAB_ASCF		; table for "F"
	!16	TAB_ASCG		; table for "G"
	!16	TAB_ASCH		; table for "H"
	!16	TAB_ASCI		; table for "I"
	!16	TAB_ASCL		; table for "L"
	!16	TAB_ASCM		; table for "M"
	!16	TAB_ASCN		; table for "N"
	!16	TAB_ASCO		; table for "O"
	!16	TAB_ASCP		; table for "P"
	!16	TAB_ASCR		; table for "R"
	!16	TAB_ASCS		; table for "S"
	!16	TAB_ASCT		; table for "T"
	!16	TAB_ASCU		; table for "U"
	!16	TAB_ASCV		; table for "V"
	!16	TAB_ASCW		; table for "W"
	!16	TAB_POWR		; table for "^"
...
TAB_LESS
LBB_LSHIFT
	!8	"<",TK_LSHIFT	; <<	note - "<<" must come before "<"
	!8 TK_LT			; <
	!8	$00
TAB_EQUL
	!8 TK_EQUAL,$00	; =
TAB_MORE
LBB_RSHIFT
	!8	">",TK_RSHIFT	; >>	note - ">>" must come before ">"
	!8 TK_GT			; >
	!8	$00
TAB_QEST
	!8 TK_PRINT,$00	; ?
...
LBB_BINS
	!raw	"IN$(",TK_BINS	; BIN$(
LBB_BITCLR
	!raw	"ITCLR",TK_BITCLR	; BITCLR
LBB_BITSET
	!raw	"ITSET",TK_BITSET	; BITSET
LBB_BITTST
	!raw	"ITTST(",TK_BITTST
					; BITTST(
	!8	$00
TAB_ASCC
LBB_CALL
	!raw	"ALL",TK_CALL	; CALL
LBB_CHRS
	!raw	"HR$(",TK_CHRS	; CHR$(
LBB_CLEAR
	!raw	"LEAR",TK_CLEAR	; CLEAR
LBB_CONT
	!raw	"ONT",TK_CONT	; CONT
LBB_COS
	!raw	"OS(",TK_COS	; COS(
	!raw	$00
TAB_ASCD
LBB_DATA
	!raw	"ATA",TK_DATA	; DATA
LBB_DEC
	!raw	"EC",TK_DEC		; DEC
LBB_DEEK
	!raw	"EEK(",TK_DEEK	; DEEK(
LBB_DEF
	!raw	"EF",TK_DEF		; DEF
LBB_DIM
	!raw	"IM",TK_DIM		; DIM
LBB_DOKE
	!raw	"OKE",TK_DOKE	; DOKE note - "DOKE" must come before "DO"
LBB_DO
	!raw	"O",TK_DO		; DO
	!raw	$00
TAB_ASCE
LBB_ELSE
	!raw	"LSE",TK_ELSE	; ELSE
LBB_END
	!raw	"ND",TK_END		; END
LBB_EOR
	!raw	"OR",TK_EOR		; EOR
...
LBB_GET
	!raw	"ET",TK_GET		; GET
LBB_GOSUB
	!raw	"OSUB",TK_GOSUB	; GOSUB
LBB_GOTO
	!raw	"OTO",TK_GOTO	; GOTO
	!raw	$00
TAB_ASCH
LBB_HEXS
	!raw	"EX$(",TK_HEXS	; HEX$(
...
LBB_LCASES
	!raw	"CASE$(",TK_LCASES
					; LCASE$(
LBB_LEFTS
	!raw	"EFT$(",TK_LEFTS	; LEFT$(
LBB_LEN
	!raw	"EN(",TK_LEN	; LEN(
LBB_LET
	!raw	"ET",TK_LET		; LET
LBB_LIST
	!raw	"IST",TK_LIST	; LIST
LBB_LOAD
	!raw	"OAD",TK_LOAD	; LOAD
LBB_LOG
	!raw	"OG(",TK_LOG	; LOG(
LBB_LOOP
	!raw	"OOP",TK_LOOP	; LOOP
	!raw	$00
TAB_ASCM
LBB_MAX
	!raw	"AX(",TK_MAX	; MAX(
...
LBB_NEW
	!raw	"EW",TK_NEW		; NEW
LBB_NEXT
	!raw	"EXT",TK_NEXT	; NEXT
LBB_NMI
	!raw	"MI",TK_NMI		; NMI
LBB_NOT
	!raw	"OT",TK_NOT		; NOT
LBB_NULL
	!raw	"ULL",TK_NULL	; NULL
	!raw	$00
TAB_ASCO
LBB_OFF
	!raw	"FF",TK_OFF		; OFF
...
TAB_ASCP
LBB_PEEK
	!raw	"EEK(",TK_PEEK	; PEEK(
LBB_PI
	!raw	"I",TK_PI		; PI
LBB_POKE
	!raw	"OKE",TK_POKE	; POKE
LBB_POS
	!raw	"OS(",TK_POS	; POS(
LBB_PRINT
	!raw	"RINT",TK_PRINT	; PRINT
	!raw	$00
TAB_ASCR
LBB_READ
	!raw	"EAD",TK_READ	; READ
LBB_REM
	!raw	"EM",TK_REM		; REM
LBB_RESTORE
	!raw	"ESTORE",TK_RESTORE
					; RESTORE
LBB_RETIRQ
	!raw	"ETIRQ",TK_RETIRQ	; RETIRQ
LBB_RETNMI
	!raw	"ETNMI",TK_RETNMI	; RETNMI
LBB_RETURN
	!raw	"ETURN",TK_RETURN	; RETURN
LBB_RIGHTS
	!raw	"IGHT$(",TK_RIGHTS
					; RIGHT$(
LBB_RND
...
TAB_ASCS
LBB_SADD
	!raw	"ADD(",TK_SADD	; SADD(
LBB_SAVE
	!raw	"AVE",TK_SAVE	; SAVE
LBB_SGN
	!raw	"GN(",TK_SGN	; SGN(
LBB_SIN
	!raw	"IN(",TK_SIN	; SIN(
...
LBB_SQR
	!raw	"QR(",TK_SQR	; SQR(
LBB_STEP
	!raw	"TEP",TK_STEP	; STEP
LBB_STOP
	!raw	"TOP",TK_STOP	; STOP
LBB_STRS
	!raw	"TR$(",TK_STRS	; STR$(
LBB_SWAP
	!raw	"WAP",TK_SWAP	; SWAP
	!raw	$00
TAB_ASCT
LBB_TAB
	!raw	"AB(",TK_TAB	; TAB(
LBB_TAN
	!raw	"AN(",TK_TAN	; TAN(
LBB_THEN
	!raw	"HEN",TK_THEN	; THEN
LBB_TO
	!raw	"O",TK_TO		; TO
LBB_TWOPI
	!raw	"WOPI",TK_TWOPI	; TWOPI
	!raw	$00
TAB_ASCU
LBB_UCASES
	!raw	"CASE$(",TK_UCASES
...
LBB_VAL
	!raw	"AL(",TK_VAL	; VAL(
LBB_VPTR
	!raw	"ARPTR(",TK_VPTR	; VARPTR(
	!raw	$00
TAB_ASCW
LBB_WAIT
	!raw	"AIT",TK_WAIT	; WAIT
LBB_WHILE
	!raw	"HILE",TK_WHILE	; WHILE
LBB_WIDTH
	!raw	"IDTH",TK_WIDTH	; WIDTH
	!raw	$00
TAB_POWR
	!raw	TK_POWER,$00	; ^
...
LAB_KEYT
	...
	!16	LBB_IF		; IF
	!8	7,'R'
	!16	LBB_RESTORE		; RESTORE
	!8	5,'G'
	!16	LBB_GOSUB		; GOSUB
	!8	6,'R'
	!16	LBB_RETIRQ		; RETIRQ
	!8	6,'R'
	!16	LBB_RETNMI		; RETNMI
	!8	6,'R'
	!16	LBB_RETURN		; RETURN
	!8	3,'R'
	!16	LBB_REM		; REM
	!8	4,'S'
	!16	LBB_STOP		; STOP
	...
	!16	LBB_SWAP		; SWAP
	!8	6,'B'
	!16	LBB_BITSET		; BITSET
	!8	6,'B'
	!16	LBB_BITCLR		; BITCLR
	!8	3,'I'
	!16	LBB_IRQ		; IRQ
	!8	3,'N'
	!16	LBB_NMI		; NMI
...
LAB_BAER
	!16	ERR_NF		;$00 NEXT without FOR
	!16	ERR_SN		;$02 syntax
	!16	ERR_RG		;$04 RETURN without GOSUB
	!16	ERR_OD		;$06 out of data
	!16	ERR_FC		;$08 function call
	!16	ERR_OV		;$0A overflow
	!16	ERR_OM		;$0C out of memory
	!16	ERR_US		;$0E undefined statement
	!16	ERR_BS		;$10 array bounds
	!16	ERR_DD		;$12 double dimension array
	!16	ERR_D0		;$14 divide by 0
	!16	ERR_ID		;$16 illegal direct
	!16	ERR_TM		;$18 type mismatch
	!16	ERR_LS		;$1A long string
	!16	ERR_ST		;$1C string too complex
	!16	ERR_CN		;$1E continue error
	!16	ERR_UF		;$20 undefined function
	!16 ERR_LD		;$22 LOOP without DO
...
ERR_NF	!raw	"NEXT without FOR",$00
ERR_SN	!raw	"Syntax",$00
ERR_RG	!raw	"RETURN without GOSUB",$00
ERR_OD	!raw	"Out of DATA",$00
ERR_FC	!raw	"Function call",$00
ERR_OV	!raw	"Overflow",$00
ERR_OM	!raw	"Out of memory",$00
ERR_US	!raw	"Undefined statement",$00
ERR_BS	!raw	"Array bounds",$00
ERR_DD	!raw	"Double dimension",$00
ERR_D0	!raw	"Divide by zero",$00
ERR_ID	!raw	"Illegal direct",$00
ERR_TM	!raw	"Type mismatch",$00
ERR_LS	!raw	"String too long",$00
ERR_ST	!raw	"String too complex",$00
ERR_CN	!raw	"Can't continue",$00
ERR_UF	!raw	"Undefined function",$00
ERR_LD	!raw	"LOOP without DO",$00

;ERR_UV	!raw	"Undefined variable",$00

; the above error has been tested and works (see code and comments below LAB_1D8B)

;ERR_UA	!8	"Undimensioned array",$00

LAB_BMSG	!raw	$0D,$0A,"Break",$00
LAB_EMSG	!raw	" Error",$00
LAB_LMSG	!raw	" in line ",$00
LAB_RMSG	!raw	$0D,$0A,"Ready",$0D,$0A,$00
